...
namespace ColorShapeLinks.UnityApp
{
    ...
    public class MatchView : MonoBehaviour
    {
        ...
        {
            // Keep track of time which animation started and time that it
            // should end
            ...
            // Animate while we have time, avoiding sudden breaks in the
            // animation
            while (Time.time < timeToEnd || color.a < 0.95f)
            {
                color.a =
                    Mathf.Cos(Time.time * Mathf.PI * 2 / lastMoveAnimLength)
                    * 1f / 2f + 1f / 2f;
        ...
        /// <summary>
        /// Submit a message which will be shown in the message box when
        /// possible. However, message will be shown in the Unity console
        /// immediately.
        /// </summary>
            ...
            while (true)
            {
                ...
                if (messageQueue.Count > 0)
                {
                    ...
                    while (messageQueue.Count > 0)
                    {
                        ...
                        // Get message out of the queue and into the string
                        // builder
                        messages.Append($"\n- {messageQueue.Dequeue()}");
                        // If the string is too long, this may cause an
                        // exception in Unity, so we make sure it isn't.
                        // This is not the best solution, but it works as long
                        // as the user doesn't want to see everything since the
                        // start. Better solutions here:
        ...
        /// <summary>
        /// Native C# event which will be invoked when a move is selected in
        /// the UI.
        /// </summary>
